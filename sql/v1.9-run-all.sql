...
-- CWA Ride Scheduler — v1.9 Phase 2 RUN
-- This script assumes Phase 1 has run and created schema cwa and base objects.
-- It adds helper functions, RPCs, and deferrable constraint triggers for:
--   - State transitions & operating-hours checks (save_ride)
--   - Assign/unassign people with role/status gating and availability (assign_person, unassign_person)
--   - Per-passenger EC links with availability checks (link_emergency_contact, unlink_emergency_contact)
--   - Composition (exactly 1 pilot, ≤ 2 passengers) — deferrable constraint
--   - Overlap prevention per person across rides — deferrable constraint
-- All write RPCs are SECURITY DEFINER and gated by is_admin_or_scheduler().

BEGIN;
SET search_path = cwa, public;

-- === 0. Safety: ensure needed Phase 1 objects exist (soft checks) ====================
-- (No-op if present; raises helpful error if missing.)

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables
                 WHERE table_schema='cwa' AND table_name='ride_events') THEN
...
-- Simple settings function (can be replaced by a table later).
-- Returns local window as time-of-day in America/Los_Angeles.
...
-- Convert timestamptz to local TIME in America/Los_Angeles
...
-- Verify a [start,end) range is within the configured same-day local window
...
-- Is the given person currently allowed for the role per role_allowed_statuses?
...
-- Is pilot assignable (must be active, not merely in_training)
...
-- Build warnings array for a would-be pilot assignment (cert warn-only)
...
-- Check if a person is unavailable for [start,end)
...
BEGIN
  ...
  ELSE
    -- immutability on historical rides
    IF v_existing.end_at < v_now THEN
      -- allow scheduled->completed within 24h
...
BEGIN
  ...
  -- stricter pilot rule: must be active
  ...
  -- availability (unavailability blocks assignment)
  ...
  -- Insert assignment (deferred composition/overlap enforcement will run at commit)
...
-- === 6. RPC: Emergency contact linkage (Option B) ==================================
...
-- 7.1 Composition: exactly 1 pilot, ≤2 passengers per ride
...
-- 7.2 Overlap prevention: no person assigned to overlapping rides
...
-- Also re-check overlaps if a ride time changes
